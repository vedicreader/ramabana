...
- **Until Task 3 lands, a bare `uv run pytest -q` cannot run.** `tests/test_pyrepl.py` imports
  `Kernel`, `DhrishtiHost`, `output_text` and `cli` at module scope, so the file fails at
  *collection* and aborts the whole run while any of those is missing. For Tasks 1 and 2 the
  regression check is `uv run pytest -q --ignore=tests/test_pyrepl.py` (expect 163 passed) plus
  whatever targeted verification the task names. From Task 3 on, use the bare `uv run pytest -q`.
...
- [ ] **Step 0 (do this first): settle the `vault` parameter**

Task 4 found that `mk_pyagent` accepts `vault` and silently drops it: `cli.mk_host` uses `vault` to
*choose* between `LocalHost` and `VaultHost` by class and never passes it as a keyword, and
`LocalHost.__init__` has no `vault` parameter. So `ramabana pyrepl --vault` today runs without a
vault and says nothing — a flag that lies.

A vault-backed `DhrishtiHost` is a new feature and is not in this spec. So **drop it**: remove
`vault` from `mk_pyagent`, `amain` and `main` in `nbs/11_pyrepl.ipynb`, and remove `vault=vault`
from the `prompt == 'pyrepl'` dispatch in `nbs/05_cli.ipynb`. Leave `cli.main`'s own `vault`
parameter alone — the ordinary agent still supports it.

Then fix the Task 4 test cell that asserts `_seen['vault'] is False`: it must no longer expect a
key the dispatch does not send. Change that line to assert the key is absent:

```python
assert 'vault' not in _seen      # pyrepl has no vault-backed host, so the flag is not offered
```

Run `uv run pytest -q` (expect 167 passed) and `uv run nbdev-test --path nbs/11_pyrepl.ipynb`
before moving on.
