...
ui.buf.insert('comp'); ui.buf.cursor = 4
await ui.complete_python()
assert ui.matches is None or any('→' in m for m in ui.matches) or len(ui.matches) > 1
ui.buf.clear()
```
...
```bash
git add nbs/11_pyrepl.ipynb ramabana/ nbs/llms.txt nbs/sidebar.yml CHANGELOG.md
