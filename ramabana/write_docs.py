...
A summary of a long source -- a conversation, a diff, a review thread, a paper -- has to say what is now true, not shorten the source in its own order. Write from the situation the source describes, not from the source. Six failures cover nearly all bad summaries.
...
- No status and no why: a thread reported without its current state (decided, dropped, still open; merged, blocked, awaiting review), or a conclusion kept while the reason that supports it is dropped. Gathering the reasons scattered across a long source is most of the work of summarizing, which is why lazy summaries lose them first.
- Space by bulk and buried lede: length tracking the size of the source material instead of how much it changed the situation, with the biggest change arriving last because that is where the source got to it. Weight by importance and put the largest change first.
...
To have a draft checked against these rules, send the rules and the draft to a subagent with `delegate_parallel`, naming the audience so tell 12 can be judged, and ask for flagged spans rather than a rewrite. Only do that when the user asks for a docs check.
