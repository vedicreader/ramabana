...
# %% auto #0
__all__ = ['MAX_DETAIL', 'MAX_ACTS', 'ICONS', 'DENIED', 'DFLT_TIMEOUT', 'MAX_PREVIEW', 'INLINE_SKILLS', 'COMPLETE_SP',
           ...
           'note', 'tool_plan', 'request_text', 'prompt_directives', 'system_prompt', 'Agent', 'Completer']

# %% ../nbs/03_agent.ipynb #ace94f1a
import datetime, functools, json, re, threading, time, uuid
from dataclasses import dataclass, field
from .core import agent_err, JOBS, Routing, model_note
from .runtime import Usage, make_backend, Compactor, compact_notebook_context, notices_block
from .tools import MAX_TOOL_CHARS, WRITE_TOOLS, Registry, discover, find, load, skill_index, subagent_tools, tools_for, clip
...
def tool_plan(prompt):
    ...
    This is intentionally not another model call: routing “what default model does leela
    use?” to the web is exactly the mistake this prevents, and tool choice should be
    predictable enough to inspect in conversation history.
    """
    p = str(prompt or '').lower()
    repo = ('leela', 'this repo', 'repository', 'codebase', 'implementation', 'implemented',
    ...
    if any(x in p for x in repo):
        return ('repo', 'Use search_code first. Read the matching source if needed. '
                        'Do not web-search a question about Leela or the open repository.')
...
How to work:
- Act on the user's verb. “Create”, “scale”, “run”, “fix”, and “as NAME” request a result,
  not a plan: use the appropriate write/run tool, verify the result, then report what exists.
  Never stop at “I will…” and never claim code ran unless a tool result confirms it.
- Use `search_code` for project behavior, unfamiliar APIs, or uncertainty about an installed
  library. Do not search for routine Python/pandas operations you already know; execute and
  verify them instead. Use `web_search` only when current external documentation matters.
- Prefer `view_file` over guessing. Its output doubles as the address book for edits.
  Copy paths exactly from the workspace, notebook context, or `search_code`; never shorten,
  repair, or reconstruct an absolute path.
- A `<notebook path="...">` block is the currently open notebook and its exact path. Its
  cells shown there are already visible; use that path with notebook tools and never reconstruct it.
- Never use `view_file` or `edit_file` on an `.ipynb` file. Call `notebook_cells` on its exact
  path, choose the returned cell id, then `view_cell` and `edit_cell`. Those cell views are
  exhash address books just like file views.
...
- Code cells inside `<notebook>` have already executed. Their printed `<output>` is not
  Python and must never be copied into `run_python`. For a request about `df`, first call
  `list_vars`, then run only the new transformation the user requested.
- For “scale numeric columns of SOURCE ... as OUTPUT”, call `scale_numeric` directly;
  do not search and do not reconstruct the DataFrame's setup code.
- `run_python` shares the user's kernel namespace. Read anything; bind results to NEW
  names. You cannot rebind or delete the user's variables, so do not try. If a tool fails,
  correct the call and continue; do not replace execution with a plan.
...
class Agent:
    ...
    def set_model(self, name, job='turn'):
        "Point `job` at `name`, releasing its old engine only when no other job shares it."
    ...
    def _prepare(self, prompt):
        ...
        if self.compactor.auto and (self.compactor.due(b) or not b.fits(outgoing)):
            self.compact()
    ...
    def _load_history(self):
        ...
        try:
            # History is the user's archive, not model context. Keep enough to show prior
            # agent/model sessions; the old 100-turn tail made older Luna conversations
            # silently disappear from the History panel.
            self.history = [json.loads(line) for line in p.read_text().splitlines() if line.strip()][-2000:]
    ...
    def _finish(self, text, prompt=''):
        ...
        if self.current_turn_id in self.checkpoints:
            self.checkpoints[self.current_turn_id]['after'] = b.snapshot_hist()
    ...
    def compose(self, prompt, context='', screen='', image=None, context_path=''):
        ...
        if local_text_only:
            parts.append('[Image attachment omitted: local multimodal is disabled.]')
        if context:
