...
#: What a tool block folds to once its result arrives: the step's own summary line, whatever
#: the result's height. The step is the message and the output is one ctrl-O away, so a turn
#: of thirty calls reads as thirty lines. `FOLD` cannot express this by itself, because a tool
#: block is born one hourglass row tall and only grows its result later, by which time the
#: born-folded decision has already been made against the wrong height.
FOLD_TOOL = 1
...
READING THE SCREEN
  ...
  Blocks stack, one per thing that happened. A tool call is one row,
  however much it returned; any other block over 12 rows arrives folded.
  Both say how many rows they hide, and ctrl+o unfolds the last one.
