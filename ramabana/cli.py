...
# %% auto #0
__all__ = ['DARK', 'LIGHT', 'THEMES', 'CODE_THEMES', 'KAKU', 'GRUVBOX', 'ACTIVE_THEME', 'MARKDOWN_THEME', 'GUTTERS', 'FOLD',
           'FOLD_TOOL', 'NOTIFY_EVERY', 'FOLD_RUNNING', 'ACT_EVERY', 'FOLD_STEP', 'STREAM_EVERY', 'ACT_TAIL',
           'MAX_GROUP_ROWS', 'MOUSE_ON', 'MOUSE_OFF', 'SURFACE_COMMANDS', 'HELP', 'BOW', 'WORDMARK', 'BANNER_GAP',
           'BANNER_MIN', 'ARROW', 'MARK_WIDTH', 'SLACK', 'FLIGHT', 'FLIGHT_WIDTH', 'BUILD', 'VERSION', 'GUIDE', 'MEDIA',
           ...
           'MENUS', 'PYREPL_MODULES', 'code_theme', 'set_theme', 'plan_text', 'banner', 'arrow_mark', 'flight_line',
...
#: and every palette names one, so the fallback in `code_theme` is for a name nothing recognises.
CODE_THEMES = {'dark': 'github-dark', 'gruvbox': 'gruvbox-dark',
               'gruvbox-light': 'gruvbox-light', 'nord': 'nord',
...
extra   /root · /root add PATH to open another folder · /theme [NAME|next|prev] · /mouse to click blocks on the main screen · /tool-budget · /steps · /models · /model NAME · /sessions · /resume [ID|latest] · /cost · /compact · /reload
...
def _fit(text, room):
    """`text` inside `room` terminal cells, an ellipsis where it was cut. `room` None means no limit.

    Cells, not codepoints: a note can carry CJK or an emoji, either of which is two cells wide, and
    slicing by character let a clipped note overrun the width it had just been clipped to.
    """
    if room is None or cell_len(text) <= room: return text
    if room < 2: return ''
    out = ''
    for ch in text:
        if cell_len(out) + cell_len(ch) > room - 1: break
        out += ch
    return out + '…'
...
def banner(note='',        # what the session has to say for itself: the model, or why there is none
    ...
    That last line is the name, eight cells of it, whatever `width` said -- there is nothing shorter
    ...
    # Flatten it once here; each tier then clips to the room *it* has, which is not the same room.
    note = ' '.join(str(note).split()) if note else ''
    ...
    if width is not None and width < BANNER_MIN:
        ...
        tail = _fit(note, width)                 # its own row, so the whole width is its to use
        if tail: out.append(f'\n{tail}', style=quiet)
        return out
...
class Ui:
    ...
    def working(self):
        ...
        # One fixed track, or none. A track sized from what is left over changed width as `9s`
        # became `10s`, which changed the cycle it wraps on and made the arrow jump a cell.
        if cell_len(line.plain) + FLIGHT_WIDTH + 3 <= self.comp.console.width - 2:
            line.append('   ')
            line.append(flight_line(self.frame, FLIGHT_WIDTH))
        rows.append(line)
        return rows
...
def apply_theme(self:Ui, name=''):
    ...
    names = list(THEMES)
    name = str(name or '').lower()   # before the keyword test, or `/theme NEXT` is an unknown theme
